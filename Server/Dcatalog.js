...
// Obtener el nombre del dinosaurio desde los argumentos de la línea de comandos
const dinoName = process.argv[2];
const excelFilePath = 'C:\\Users\\CIJHU11\\Desktop\\Juan\\Otros\\APPs\\Seguridad\\BDD Filtrada.xlsx';
if (dinoName) {
    if (fs.existsSync(excelFilePath)) {
        const dinoData = readDinoDataFromExcel(excelFilePath);
        searchDinosaurByName(dinoName, dinoData).then(data => console.log(data));
    } else {
        console.log('El archivo Excel no se encontró en la ruta especificada.');
    }
} else {
    console.log('Por favor, proporciona el nombre de una especie de dinosaurio.');
}

// Al final del archivo Dcatalog.js
module.exports = {
